...
---
= Associe as credenciais do agente do Console à sua assinatura do Google Cloud
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Ao criar um agente do Console no Google Cloud, você pode usar as credenciais associadas para assinar o NetApp Intelligent Services, que inclui o Cloud Volumes ONTAP e os serviços de dados do NetApp .  Você deve associar a assinatura ao agente cujas credenciais você deseja usar para pagar pelos serviços.
...
Substituir a assinatura atual do marketplace por uma nova atualiza a assinatura dos sistemas Cloud Volumes ONTAP existentes e novos.
...
A assinatura dos serviços de dados da NetApp por meio do Google Cloud Marketplace pode falhar devido a permissões incorretas ou redirecionamento perdido para o Console. Se isso acontecer, siga as etapas a seguir para concluir o processo de assinatura.
