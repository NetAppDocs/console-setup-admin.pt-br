...
---
= Alternar entre organizações, projetos e agentes do NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
TIP: Você pode pertencer a várias organizações se outra organização o convidar para participar ou se você mesmo criar uma.  Você pode criar uma organização adicional usando a API. https://docs.netapp.com/us-en/console-automation/tenancyv4/post-organizations.html["Aprenda a criar uma nova organização"^]
...
+
A aba *Parceria* não é exibida se você não tiver nenhuma organização parceira.
