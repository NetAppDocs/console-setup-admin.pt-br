...
---
= Implante um agente do NetApp Console a partir do AWS Marketplace
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Você implanta um agente do Console na AWS diretamente do AWS Marketplace. Para implantar um agente do Console a partir do AWS Marketplace, você precisa configurar sua rede, preparar as permissões da AWS, revisar os requisitos da instância e, em seguida, implantar o agente do Console.
...
.Antes de começar
* link:concept-agent-install-options.html#agent-marketplace-deploy["Saiba quando implantar a partir do AWS Marketplace"].
...
Abra um navegador da web e vá para o https://console.netapp.com["NetApp Console"^] para começar a usar o agente do Console com o Console.
