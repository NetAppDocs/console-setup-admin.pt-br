...
---
= Implante manualmente um agente do Console na AWS para o NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
Dependendo dos NetApp data services que você planeja usar, talvez seja necessário criar uma segunda política. Para regiões padrão, as permissões são distribuídas em duas políticas. Duas políticas são necessárias devido ao limite máximo de caracteres para políticas gerenciadas na AWS. link:reference-permissions-aws.html["Saiba mais sobre as políticas do IAM para o agente do Console"].
...
Se você tiver buckets do Amazon S3 na mesma conta da AWS em que criou o agente do Console, verá um sistema de storage Amazon S3 aparecer automaticamente na página *Sistemas*. https://docs.netapp.com/us-en/storage-management-s3-storage/index.html["Aprenda a gerenciar buckets do S3 no NetApp Console"^]
