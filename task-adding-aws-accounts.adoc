...
[role="lead"]
Adicione e gerencie credenciais da AWS para que você possa implantar e gerenciar recursos na nuvem em suas contas da AWS a partir do NetApp Console. Se você gerencia várias assinaturas do AWS Marketplace, pode atribuir cada uma delas a credenciais diferentes da AWS na página Credenciais.
...
link:concept-accounts-aws.html["Saiba como o NetApp Console usa credenciais e permissões da AWS"].
...
NOTE: Se você implantou um agente do Console a partir do Console, ele adicionou automaticamente credenciais da AWS para a conta na qual você implantou um agente do Console. Isso garante que as permissões necessárias estejam em vigor para gerenciar recursos.
...
Se você deseja fornecer chaves da AWS para um usuário do IAM ao Console, é necessário conceder as permissões necessárias a esse usuário. A política do IAM do Console define as ações e os recursos da AWS que o Console tem permissão para usar.
...
NOTE: Novas credenciais no seu provedor de nuvem podem levar alguns minutos para ficarem disponíveis.
...
Configure uma função do IAM que permita que o aplicativo de software como serviço (SaaS) NetApp Console assuma a função.
...
----
+
Consulte link:https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies-cross-account-resource-access.html["Documentação do AWS Identity and Access Management (IAM)"^] para obter mais informações sobre o acesso a recursos entre contas no IAM.
...
.Resultado
A função IAM agora tem as permissões necessárias. <<add-the-credentials,Agora você pode adicioná-lo ao Console>>.
...
. Na página *Credenciais da organização*, selecione *Adicionar credenciais* e siga as etapas do assistente.
+
.. *Localização das credenciais*: Selecione *Amazon Web Services > Console*.
...
Para obter detalhes, consulte o https://docs.netapp.com/us-en/storage-management-fsx-ontap/requirements/task-setting-up-permissions-fsx.html["Documentação do console para Amazon FSx para ONTAP"^]
...
.Antes de começar
Você precisa criar um agente de Console antes de configurar uma assinatura. link:concept-agent-install-options.html["Aprenda a criar um agente de console"].
...
Ao assinar um serviço no AWS Marketplace, a última etapa do processo é associar a assinatura à sua organização. Se você não concluir essa etapa, não poderá usar a assinatura com sua organização.
...
.Passos
. Confirme se você não associou sua assinatura à sua organização do Console.
+
...
. Confirme se a assinatura está associada à sua organização.
+
...
.Passos
. Selecione *Administração > Credenciais*.
. Na página *Credenciais da organização*, selecione o menu de ações para um conjunto de credenciais e, em seguida, selecione *Editar credenciais*.
...
TIP: Não é possível excluir as credenciais de um perfil de instância associado a um agente do Console.
