...
Não é possível carregar as configurações do agente
 a| 
O Console exibe esta mensagem quando você tenta acessar a página de configurações do Agent.
 a| 
...
Esta mensagem é exibida se o agente não conseguir acessar sua conta de suporte.
 a| 
* Verifique se o agente tem acesso de saída aos endpoints necessários.link:reference-networking-saas-console.html["Saiba mais sobre os pontos de extremidade de acesso à rede necessários."]
...
Use um dos seguintes comandos para verificar o agente do Console. Todos os serviços devem ter o status _Em execução_. Se isso não acontecer, entre em contato com o suporte da NetApp.
...
* link:task-agent-vm-config.html#connect-vcenter-host["Verifique o status do agente do Console (para vCenter deployments)"]


====
Docker (para Ubuntu e vCenter deployments)::
+
--
[source, cli]
----
...
----
--
Podman (para implantações do Red Hat Enterprise Linux)::
+
--
[source, cli]
----
...
== Verifique se o agente do Console atende aos requisitos

Os agentes de console exigem acesso a determinados endpoints de rede, precisam ter várias portas abertas e devem atender aos requisitos de hardware. Você pode verificar se o seu agente de console atende a esses requisitos no Console ou usando o agent maintenance console.
...
Você também pode executar verificações de validação usando o console de manutenção do agente. link:reference-agent-maint-console-config-checker.html["Saiba mais sobre como usar o comando `config-checker validate`."]


IMPORTANT: Para clientes de nuvem do governo (modo restrito), o NetApp Console não valida o endpoint de upgrade do modo restrito. Você deve confirmar manualmente se as regras do seu firewall permitem acesso a esse endpoint.link:task-prepare-restricted-mode.html["Saiba mais sobre os pontos de acesso à rede necessários para modo restrito."]


NOTE: Você só pode validar agentes que tenham o status *Conectado*.

.Etapas a partir do console
. Selecione *Administração > Agentes*.
. Selecione o menu de ações do agente do Console que deseja verificar e escolha *Validar*.
+
image:screenshot-agent-validate.png["Uma captura de tela que mostra a opção Validar no menu Ações para um agente."]
...
Se você não conseguiu resolver os problemas com o agente do Console, talvez queira entrar em contato com o suporte da NetApp. O suporte da NetApp pode solicitar o ID do agente do Console ou pedir que você envie os logs do agente do Console, caso ainda não os tenham.
...
Se você estiver com problemas, a NetApp pode pedir que você envie uma mensagem AutoSupport para o Suporte da NetApp para fins de resolução de problemas.
