...
summary: Você pode fazer alterações no seu vCenter ou host ESXi existente após implantar o agente do Console. Por exemplo, você pode aumentar a CPU ou a RAM da instância da VM que hospeda o agente do Console. 
---
= Mantenha um host vCenter ou ESXi para o agente do NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Você pode fazer alterações no seu vCenter ou host ESXi existente após implantar o agente do Console. Por exemplo, você pode aumentar a CPU ou a RAM da instância da VM que hospeda o agente do Console.
...
Visualize as configurações de rede da VM do agente no vSphere client para confirmar ou solucionar problemas de rede. Você só pode visualizar (não atualizar) as seguintes configurações de rede: endereço IP e detalhes de DNS.
