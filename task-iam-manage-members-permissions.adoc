...
Você pode adicionar dois tipos de membros à sua organização: uma conta de usuário e uma conta de serviço. Os aplicativos usam contas de serviço para executar tarefas de API automaticamente.  As pessoas usam contas de usuário para fazer login e gerenciar recursos.
...
Ao remover um membro, o sistema revoga suas permissões, mas mantém suas contas no Console e no Site de Suporte da NetApp .
...
. Selecione *Recriar*.
. Baixe ou copie o ID do cliente e o segredo do cliente. + O console exibe o segredo do cliente apenas uma vez. Copie ou baixe e armazene com segurança.
...
Os usuários deverão configurar a autenticação multifator (MFA) novamente ao fazerem login após a remoção. Se o usuário tiver perdido o acesso ao seu dispositivo MFA apenas temporariamente, ele poderá usar o código de recuperação que salvou quando configurou o MFA para fazer login.
