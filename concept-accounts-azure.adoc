...
Ao implantar um agente do Console a partir do Console, você precisa usar uma conta do Azure ou uma entidade de serviço que tenha permissões para implantar a máquina virtual do agente do Console.  As permissões necessárias estão listadas emlink:task-install-agent-azure-console.html#agent-custom-role["Política de implantação de agente para o Azure"] .
...
image:diagram_permissions_multiple_azure.png["Uma imagem conceitual que mostra a conta inicial do Azure que recebe permissões por meio de uma função personalizada e identidade gerenciada e duas contas adicionais que recebem permissões por meio de uma função personalizada e service principal."]
