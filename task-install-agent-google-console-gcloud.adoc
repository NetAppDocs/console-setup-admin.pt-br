...
summary: Para criar um agente do Console no Google Cloud a partir do NetApp Console ou usando o Google Cloud, você precisa configurar sua rede, preparar as permissões do Google Cloud, ativar as APIs do Google Cloud e, em seguida, criar o agente do Console. 
---
= Implante um agente do Console no Google Cloud a partir do NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Você pode implantar um agente do Console no Google Cloud a partir do Console. Você precisa configurar sua rede, preparar as permissões do Google Cloud, ativar as APIs do Google Cloud e, em seguida, implantar o agente do Console.

Ao implantar um agente no Google Cloud, é iniciada uma instância de máquina virtual executando Linux e o software do agente Console em uma VPC de sua escolha.

.Antes de começar
* link:concept-agent-install-options.html#agent-console-deploy["Saiba quando implantar a partir do Console"].
...
Endpoints contatados do console NetApp:: Ao usar o aplicativo SaaS Console baseado na Web NetApp, ele contata vários endpoints para concluir tarefas de gerenciamento de dados. Isso inclui endpoints contatados para implantar o agente do Console a partir do Console.
+
--
...
== Etapa 3: Crie uma conta de serviço do Google Cloud para usar com o agente.
...
. Se você planeja implantar sistemas Cloud Volumes ONTAP em projetos diferentes daquele em que o agente do Console reside, será necessário conceder à conta de serviço do agente do Console acesso a esses projetos.
+
...
. `deploymentmanager.editor` Só é necessário no projeto host se você não estiver passando regras de firewall para a implantação e optar por deixar o Console criá-las para você. O NetApp Console cria uma implantação no projeto host que contém a regra de firewall VPC0 se nenhuma regra for especificada.
. `firewall.create` e  `firewall.delete` são necessárias apenas se você não estiver passando regras de firewall para a implantação e optar por deixar o Console criá-las para você. Essas permissões residem no arquivo .yaml da conta do Console. Se você estiver implantando um par de HA usando uma VPC compartilhada, essas permissões serão usadas para criar as regras de firewall para VPC1, 2 e 3. Para todas as outras implantações, essas permissões também serão usadas para criar regras para VPC0.
. Para o Cloud Tiering, a conta de serviço de tiering deve ter a  `serviceAccount.user` função na conta de serviço, não apenas no nível do projeto. Se você atribuir  `serviceAccount.user` no nível do projeto, as permissões não são exibidas ao consultar a conta de serviço com  `getIAMPolicy`.
...
NOTE: Ao implantar um agente no Google Cloud, o agente cria um bucket para armazenar os arquivos de implantação.
...
* Uma VPC e uma sub-rede que atendam aos requisitos de rede.
...
** *Etiquetas de rede*: Adicione uma etiqueta de rede à instância do agente do Console se estiver usando um proxy transparente. As etiquetas de rede devem começar com uma letra minúscula e podem conter letras minúsculas, números e hífens. As etiquetas devem terminar com uma letra minúscula ou um número. Por exemplo, você pode usar a etiqueta  `console-agent-proxy`.
...
Se você tiver buckets do Google Cloud Storage na mesma conta do Google Cloud em que criou o agente do Console, um sistema do Google Cloud Storage aparecerá automaticamente na página *Sistemas*. https://docs.netapp.com/us-en/storage-management-google-cloud-storage/index.html["Aprenda a gerenciar o Google Cloud Storage pelo Console"^]
