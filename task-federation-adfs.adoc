...
---
= Federar o NetApp Console com os Active Directory Federation Services
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Você pode configurar a federação com seu servidor AD FS para habilitar o logon único (SSO) para o NetApp Console. O processo envolve configurar seu AD FS para confiar no Console como provedor de serviços e, em seguida, criar a conexão no NetApp Console.
...
+
.. Crie a relação de confiança usando PowerShell executando o seguinte script:
+
[source, powershell]
----
...
+
*** Ao criar o Identificador de Confiança, use o valor *YOUR_TENANT*: `netapp-cloud-account`
