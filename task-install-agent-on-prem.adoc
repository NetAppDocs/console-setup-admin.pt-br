...
summary: Implante um agente do Console localmente e, em seguida, faça login e configure-o para funcionar com sua organização do Console. 
---
= Instale manualmente um agente do NetApp Console localmente
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Implante um agente do Console localmente e, em seguida, faça login e configure-o para funcionar com sua organização do Console.


NOTE: Se você for um usuário do VMware, poderá usar um OVA para implantar um agente de Console em seu vCenter. link:task-install-agent-on-prem-ova.html["Saiba mais sobre como implantar um agente em um vCenter."]

Antes de instalar, você precisará garantir que seu host (VM ou host Linux) atenda aos requisitos e garantir que o agente do Console terá acesso de saída à internet, bem como às redes de destino. Se você planeja usar NetApp data services ou storage de nuvem, como Cloud Volumes ONTAP, precisará criar credenciais em seu provedor de nuvem para adicionar ao Console, para que o agente do Console possa executar ações na nuvem em seu nome.
...
Execute o agente do Console em um host x86 que atenda aos requisitos de sistema operacional, RAM e portas. Certifique-se de que seu host atenda a esses requisitos antes de instalar o agente do Console.
...
Se você deseja usar os serviços de dados da NetApp na AWS ou no Azure com um agente do Console local, será necessário configurar as permissões no seu provedor de nuvem e, em seguida, adicionar as credenciais ao agente do Console após a instalação.
...
Ao instalar manualmente um agente de Console, você precisa preparar o ambiente da sua máquina para que ele atenda aos requisitos. Você precisará de uma máquina Linux e precisará instalar o Podman ou o Docker, dependendo do seu sistema operacional Linux.
...
. Se você associar este agente a um Cloud Volumes ONTAP, considere aumentar o número de threads associadas ao agente para melhorar o desempenho. link:https://kb.netapp.com/Cloud/ncds/nds/cvods/cvods_kbs/CVO_going_to_failed_state_frequently_in_NetApp_Console["Saiba mais sobre como aumentar o limite de threads do contêiner do agente"^].
...
** https://mysupport.netapp.com/site/products/all/details/netapp-console/downloads-tab["Site de suporte da NetApp"^] (necessário caso você ainda não tenha acesso ao Console).
+
...
NOTE: Se você deseja configurar um proxy transparente, poderá fazê-lo após a instalação. link:reference-agent-maint-console-proxy.html["Saiba mais sobre o console de manutenção do agente."].
...
Faça login no Console e associe o agente do Console à sua organização. A forma de login depende do modo em que você está usando o Console. Se estiver usando o Console no modo padrão, faça login pelo site SaaS. Se estiver usando o Console no modo restrito ou privado, faça login localmente a partir do host do agente do Console.
