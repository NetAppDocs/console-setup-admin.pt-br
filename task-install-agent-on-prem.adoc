...
---
= Implantar manualmente um agente do Console localmente
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: Se você for um usuário do VMWare, poderá usar um OVA para implantar um agente Console no seu VCenter. link:task-install-agent-on-prem-ova.html["Saiba mais sobre como implantar um agente em um vCenter."]
...
Faça login no Console e associe o agente do Console à sua organização. Como você faz login depende do modo em que está usando o Console. Se você estiver usando o Console no modo padrão, faça login pelo site SaaS. Se você estiver usando o Console no modo restrito ou privado, faça login localmente a partir do host do agente do Console.
