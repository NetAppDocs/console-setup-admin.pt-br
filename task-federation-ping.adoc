...
Você pode configurar uma conexão federada com PingFederate para habilitar o logon único (SSO) para o Console. O processo envolve configurar seu servidor PingFederate para confiar no Console como provedor de serviços e, em seguida, criar a conexão no Console.
...
*** Para o *ID de Público/Entidade*, use  `urn:auth0:netapp-cloud-account:<fed-domain-name-pingfederate>` onde <fed-domain-name-pingfederate> é o nome de domínio da federação. Por exemplo, se o seu domínio for  `example.com`, o ID de Público/Entidade seria  `urn:auth0:netapp-cloud-account:fed-example-com-pingfederate`.
