...
.14+| Crie e gerencie funções do IAM e perfis de instância para instâncias do Cloud Volumes ONTAP | iam:ListInstanceProfiles | Sim | Sim | Não 
...
.6+| Gerenciar volumes EBS que o Cloud Volumes ONTAP usa como armazenamento de backend | ec2:CriarVolume | Sim | Sim | Não 
...
.3+| Habilitar a criptografia de dados do Cloud Volumes ONTAP usando o AWS Key Management Service (KMS) | kms:Recriptografar* | Sim | Não | Não 
