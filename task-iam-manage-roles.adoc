...
TIP: Um membro que possui a função de administrador da organização não pode receber nenhuma outra função. Eles já possuem permissões em toda a organização. Um membro com a função de administrador de pasta ou projeto não pode receber outras funções dentro da pasta ou projeto em que já possui essa função. Ambas as funções dão acesso a todos os serviços dentro do âmbito a que lhes são atribuídos.
...
.. Para remover a função de um membro, selecione image:icon-delete.png["Um ícone que lembra uma lata de lixo"] Ao lado da função, clique para remover a respectiva função do membro. Você precisará confirmar a remoção.
