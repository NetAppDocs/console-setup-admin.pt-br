...
Usuários com a função *Administrador da organização* ou *Administrador de pasta ou projeto* têm a responsabilidade de atribuir funções a outros usuários. Você pode atribuir funções de acesso com base em um projeto ou pasta. Atribua funções de acesso por projeto ou pasta.  Por exemplo, atribua a função de administrador de Resiliência a Ransomware a um usuário para um projeto e a função de administrador do SnapCenter para outro.  Para conceder a um usuário a função de administrador de Resiliência a Ransomware para todos os projetos em uma pasta, atribua a função no nível da pasta.
...
Atribua funções de acordo com sua estratégia de IAM para melhorar a segurança.  As funções do IAM limitam o acesso do usuário apenas ao que é necessário.
...
Ao adicionar um membro à sua organização, você será solicitado a atribuir uma função a ele. Você pode verificar quais funções estão atribuídas a eles atualmente.
