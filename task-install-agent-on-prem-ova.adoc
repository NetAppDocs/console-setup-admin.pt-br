...
summary: Se você for um usuário do VMware, poderá usar um arquivo OVA para instalar um agente do Console em seu vCenter. O download ou a URL do OVA estão disponíveis no NetApp Console. 
---
= Instale um agente do NetApp Console localmente usando vCenter
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Se você for um usuário do VMware, poderá usar um OVA para implantar um agente do Console em seu vCenter. O download ou a URL do OVA estão disponíveis por meio do NetApp Console.


NOTE: Ao implantar um agente de Console com suas vCenter ferramentas, você pode usar o console da web da VM para executar tarefas de manutenção. link:task-agent-vm-config.html["Saiba mais sobre o console da VM para o agente."]
...
NOTE: Não é possível gerenciar recursos no Google Cloud com um agente do Console instalado localmente. Para gerenciar recursos do Google Cloud, instale um agente no Google Cloud.
...
.Resultado
Agora você deve ter chaves de acesso de usuário do IAM com as permissões necessárias. Após instalar o agente do Console, associe essas credenciais ao agente do Console a partir do Console.
...
== Instale um agente de Console em seu vCenter environment

NetApp oferece suporte à instalação do agente do Console em seu ambiente vCenter. O arquivo OVA inclui uma imagem de VM pré-configurada que você pode implantar em seu ambiente VMware. O download do arquivo ou a implantação por URL estão disponíveis diretamente no NetApp Console. Ele inclui o software do agente do Console e um certificado autoassinado.
...
. Selecione *Com OVA*.
. Escolha baixar o OVA ou copiar a URL para usar no vCenter.




[[deploy-the-agent-in-your-vcenter-environment]]
=== Implante o agente em seu vCenter ambiente

Faça login no seu ambiente vCenter para implantar o agente.

.Passos
. Faça o upload do certificado autoassinado para seus certificados confiáveis, caso seu ambiente exija. Você substituirá este certificado após a instalação.
...
+
**** NetApp recomenda deixar esta caixa desmarcada para que a instalação inclua uma verificação de configuração do agente. A verificação de configuração valida se o agente tem acesso à rede aos endpoints necessários. Se a implantação falhar devido a problemas de conectividade, você pode acessar o relatório de validação e os logs a partir do host do agente. Em alguns casos, se você tiver certeza de que o agente tem acesso à rede, pode optar por ignorar a verificação. Por exemplo, se você ainda estiver usando o link:reference-networking-saas-console-previous.html["pontos finais anteriores"] usado para atualizações do agente, a validação falha com um erro. Para evitar isso, selecione a caixa de seleção para instalar sem uma verificação de validação. link:reference-networking-saas-console-previous.html#update-endpoint-list["Aprenda como atualizar sua lista de endpoints"].
...
*** *Domínios de pesquisa*: Especifique o nome de domínio de pesquisa a ser usado ao resolver o nome do host. Por exemplo, se o FQDN for console10.searchdomain.company.com, insira searchdomain.company.com.
