...
---
= Implante um agente do Console localmente usando o VCenter
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Se você for usuário do VMWare, poderá usar um OVA para implantar um agente de Console no seu VCenter. O download ou a URL do OVA estão disponíveis no NetApp Console.


NOTE: Ao implantar um agente de console com suas ferramentas do vCenter, você pode usar o console da web da VM para executar tarefas de manutenção. link:task-agent-vm-config.html["Saiba mais sobre o console da VM para o agente."]
...
NetApp oferece suporte à instalação do agente do Console no seu ambiente VCenter. O arquivo OVA inclui uma imagem de VM pré-configurada que você pode implantar no seu ambiente VMware. Um download de arquivo ou implantação por URL está disponível diretamente no NetApp Console. Ele inclui o software do agente do Console e um certificado autoassinado.
...
=== Implante o agente em seu ambiente vCenter
