...
---
= Saiba mais sobre projetos e permissões do Google Cloud no NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Você precisa implantar um agente do Console usando uma conta do Google que tenha permissões para iniciar o agente do Console a partir do Console.
. Ao implantar o agente do Console, você será solicitado a selecionar uma  https://cloud.google.com/iam/docs/service-accounts["conta de serviço"^] para o agente. O Console obtém permissões da conta de serviço para criar e gerenciar sistemas Cloud Volumes ONTAP, gerenciar backups usando NetApp backup e recuperação, e muito mais. As permissões são concedidas ao associar uma função personalizada à conta de serviço.
...
* link:task-install-agent-google-console-gcloud.html#agent-permissions-google["Aprenda a configurar a conta de serviço"]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-deploying-gcp.html["Aprenda a implantar o Cloud Volumes ONTAP no Google Cloud e selecione um projeto"^]
