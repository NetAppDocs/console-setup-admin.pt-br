...
.Sobre esta tarefa
A criação do agente do Console no Azure Marketplace configura uma máquina virtual com uma configuração padrão. link:reference-connector-default-config.html["Saiba mais sobre a configuração padrão do agente do Console"] .
