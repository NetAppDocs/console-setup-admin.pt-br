...
image:diagram_azure_authentication.png["Imagem conceitual que mostra o Console obtendo autenticação e autorização do Microsoft Entra ID antes de poder fazer uma chamada de API. A função Console define permissões no Active Directory. Está vinculado a uma ou mais assinaturas do Azure e a um objeto de entidade de serviço que representa o aplicativo de console."]
