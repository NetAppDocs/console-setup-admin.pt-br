...
---
= Instale um certificado assinado por uma Autoridade Certificadora (CA) para acesso ao NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Ao usar o NetApp Console no modo restrito ou privado, a interface de usuário fica acessível a partir da máquina virtual do agente do Console implantada em sua região de nuvem ou no local. Por padrão, o Console usa um certificado SSL autoassinado para fornecer acesso HTTPS seguro ao console baseado na Web em execução no agente do Console.
...
. Na página *Visão geral*, selecione o menu de ação para um agente do Console e selecione *Configuração HTTPS*.
+
Detalhes sobre a exibição do certificado, incluindo a data de validade.
